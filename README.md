...
Current release: **v0.13.0** — see [`docs/release-notes/`](docs/release-notes/) for what changed.
...
- **SSH tunnels (port forwarding)** — save `-L` forwarding rules per host, toggle them live, reorder by drag, share on the LAN (bind 0.0.0.0), or paste a full `ssh` command to import rules, target, port, and key file in one go
