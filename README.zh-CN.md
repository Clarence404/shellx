...
当前版本：**v0.30.1** —— 见 [`docs/release-notes/`](docs/release-notes/) 了解本版本变更。
...
- **SFTP 文件浏览器** —— WinSCP 风格的双面板（本地 ↔ 远程），支持拖拽上传下载、目录传输、暂停/继续/取消，还能把文件直接拖出窗口放到桌面或资源管理器。传输条按"一次拖拽一行"展示——两万个文件的文件夹也只占一行，带自己的进度、速度和剩余时间，不会刷出一长串文件列表
...
- **高级设置** —— 「设置 → 高级」可调 SSH 连接超时与 keepalive、SFTP 并发数、终端回滚缓冲、隧道重连间隔与重试上限，以及日志记录到多细
