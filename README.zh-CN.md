...
当前版本：**v0.33.0** —— 见 [`docs/release-notes/`](docs/release-notes/) 了解本版本变更。
...
- **SSH 终端**（多 tab）—— 连接 Linux / BSD / macOS 服务器，主题、字体、光标样式全可定制；支持公钥（Ed25519/RSA/ECDSA）或密码认证，连接时对照 `~/.ssh/known_hosts` 验证 host key。Ctrl+Shift+C/V 复制粘贴，多行粘贴先预览再执行，右键菜单提供复制 / 粘贴 / 全选。Ctrl+Shift+F 搜索回滚缓冲，带匹配计数、全部高亮，以及区分大小写 / 全字 / 正则三个开关。保存主机的会话掉线后会在原 tab 内自动退避重连（回滚内容与布局保留），重连放弃后显示手动「重新连接」按钮
- **本地终端** —— 在 SSH 会话旁边直接打开本机 shell（Windows 默认 PowerShell，可在设置里换任意 shell）
- **串口终端** —— 串口视图实时列出 COM 口（USB 转串口带芯片名）和已保存的连接；一键 115200 8N1 快速连接，或保存完整线路参数（波特率、数据位/停止位、校验、流控）。会话直接在视图内打开，每个会话可单独切换本地回显、选择回车发送的行结束符（CR / LF / CRLF / 无），方便对接不回显的设备
...
- **SFTP 文件浏览器** —— WinSCP 风格的双面板（本地 ↔ 远程），支持拖拽上传下载、目录传输、暂停/继续/取消，还能把文件直接拖出窗口放到桌面或资源管理器。传输条按"一次拖拽一行"展示——两万个文件的文件夹也只占一行，带自己的进度、速度和剩余时间，不会刷出一长串文件列表。远程文件可用自己的编辑器打开（右键，或在「编辑」模式下双击），每次保存自动上传回服务器；编辑器和双击行为都可在设置里指定
- **FTP 视图** —— 专门的 SFTP / FTP / FTPS 连接视图，同样的双面板、传输队列和拖拽。为老设备而生：文件名编码（自动 / UTF-8 / GBK）、被动/主动模式、显式/隐式 TLS——明文 FTP 会被明确标出。已保存的 SSH 主机一键导入为 SFTP 连接。目录列表带缓存，目录树在专属连接上后台预热——连上远处的服务器之后，到处点都是瞬时的
- **命令补全** —— 在任意提示符下输入即弹出下拉：`h` 是你在这台主机敲过的命令历史，`c` 是内置常用命令词典。↑/↓ 选、Tab 上屏；直接回车永远执行你敲的原文。纯本地实现——服务器上不装任何东西，疑似携带密码的命令不会被记录
- **命令片段** —— 把值得少敲一遍的命令存起来，任何终端里 Ctrl+Shift+K 调用：筛选、回车，命令落在终端输入行上（可按片段开启"选中即执行"）。命令里的 `${名称}` 会在使用时变成待填的空、实时预览最终命令；片段库随配置导入导出走
- **会话监控** —— 通过同一条 SSH 连接查看 Linux 主机的一屏总览仪表盘：常驻 KPI 条（CPU / 内存 / Swap / 网络 / 磁盘 I/O）带趋势、1/5/15 分钟负载均衡与开机以来累计、每核热力格、带内联条的进程表、按满度染色的文件系统占用；探测到时还有 Docker 容器 tab（每容器 CPU / 内存 / 网络 / 块设备；被监控用户只能通过免密 sudo 访问守护进程时也能用）和红色「异常」tab（列出失败的 systemd 单元，附一键复制的 journalctl / 重启命令）。刷新间隔 1–30 秒可调
- **保存主机 + 系统密钥链** —— 存一次主机信息，从侧边栏或 `+` 菜单一键连接；密码存在 OS 的 keychain 里，不会明文写配置。Ctrl 点选 / Shift 选一段可以多选，一次确认批量删除
- **导入导出** —— 直接从 `~/.ssh/config` 导入主机（通配符、`Match` 块、`Include` 行会列为已跳过并说明原因，而不是悄悄丢掉），也能把整套配置打包成一个 JSON 文件带到另一台电脑：主机、隧道规则，设置可选。密码和密钥口令留在系统钥匙串里，永远不写进文件 —— 文件里只记录哪些主机曾经存过密码，导入时会告诉你哪几台需要重新输入
- **亮色 / 深色主题** —— 默认亮色；在设置里切换后整个应用（包括终端配色）联动
- **双语界面** —— 在设置里一键切换整个界面的中英文，实时生效
- **分栏** —— 把 tab 拖到某个窗格的边缘即可分栏，拖到中间是两格互换，拖到主区最外圈则整行置顶/置底、整列靠左/靠右。同方向再分保持等分；拖分隔条改比例，双击拉平。每个窗格各自切换终端 / 文件 / 隧道 / 监控
- **高级设置** —— 「设置 → 高级」可调 SSH 连接超时与 keepalive、SFTP 并发数、终端回滚缓冲、会话与隧道重连间隔及重试上限，以及日志记录到多细
- **自动更新** —— 启动时静默检查新版本（可关闭），在「设置 → 关于」显示更新横幅，一键下载并重启；发布包用 Minisign 签名
