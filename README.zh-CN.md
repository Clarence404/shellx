...
当前版本：**v0.32.0** —— 见 [`docs/release-notes/`](docs/release-notes/) 了解本版本变更。
...
- **会话监控** —— 通过同一条 SSH 连接查看 Linux 主机的一屏总览仪表盘：常驻 KPI 条（CPU / 内存 / Swap / 网络 / 磁盘 I/O）带趋势、1/5/15 分钟负载均衡与开机以来累计、每核热力格、带内联条的进程表、按满度染色的文件系统占用；探测到时还有 Docker 容器 tab（每容器 CPU / 内存 / 网络 / 块设备）和红色「异常」tab（列出失败的 systemd 单元，附一键复制的 journalctl / 重启命令）。刷新间隔 1–30 秒可调
