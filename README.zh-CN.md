...
当前版本：**v0.31.0** —— 见 [`docs/release-notes/`](docs/release-notes/) 了解本版本变更。
...
- **SSH 终端**（多 tab）—— 连接 Linux / BSD / macOS 服务器，主题、字体、光标样式全可定制；支持公钥（Ed25519/RSA/ECDSA）或密码认证，连接时对照 `~/.ssh/known_hosts` 验证 host key。Ctrl+Shift+C/V 复制粘贴，多行粘贴先预览再执行，右键菜单提供复制 / 粘贴 / 全选。保存主机的会话掉线后会在原 tab 内自动退避重连（回滚内容与布局保留），重连放弃后显示手动「重新连接」按钮
