...
当前版本：**v0.17.0** —— 见 [`docs/release-notes/`](docs/release-notes/) 了解本版本变更。
...
- **SSH 隧道（端口转发）** —— 全新的「隧道」总览页在一个视图里列出所有主机的所有转发规则；直接点启动即可开通（主机若未连接，shellx 会在后台悄悄开一条 SSH 传输），行内拖拽排序、跨主机移动规则免删除重建、局域网共享（绑定 0.0.0.0），也能粘贴 `ssh -L …` 命令批量导入
