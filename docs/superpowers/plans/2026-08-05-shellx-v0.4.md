...
- Modify: `src/App.tsx` (conditional main content: Files activity → `<RailFilesView />`, else current tab body; add Ctrl/⌘+B global hotkey)
- Create: `src/components/RailFilesView.tsx` (skeleton: says "Rail Files" + mounts `<TransferQueue />`)
- Modify: `src/App.test.tsx` (verify view-switch changes main content and hides drawer; verify Ctrl+B toggle)
...
- [ ] **Step 6: Route main content in App.tsx + add Ctrl/⌘+B hotkey**
  ...
  Add a new `useEffect` for the Ctrl/⌘+B hotkey (alongside the existing ⌘K handler):
  ```tsx
  useEffect(() => {
